...
function gulpFootnotes(options) {
  ...
  var stream = through.obj(function(file, enc, done) {
    ...
    $footnotes.each(function(index, span){
      ...
      var $parent = $span.parents('section').last();
      var $footnotes_holder = $parent.find("div[data-type='footnotes']");

      if($footnotes_holder.length === 0) {
        $footnotes_holder = $('<div data-type="footnotes">');
        $parent.append($footnotes_holder);
      }
