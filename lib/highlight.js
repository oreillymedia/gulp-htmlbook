...
module.exports = function(finished, options) {
  ...
  var stream = through.obj(function(file, enc, next) {
    ...
    var task = function($pre, lang, code_contents){
      ...
      var code_contents = $pre.html();

      // Remove cdata
      if(code_contents.indexOf("<![CDATA[") >= 0) {
        code_contents = code_contents.replace("<![CDATA[", '').replace("]]>", '');
      }
      
      var decoded = entities.decodeHTML(code_contents); // Decode entities before passing to pygmnetize

      tasks.push(task($pre, lang, decoded));
