...
function Mapper(map, logger) {
  this.map = map || {};
  this.counter = new Counter();
  this.log = logger || gutil.log;
};
...
Mapper.prototype.parse = function(contents, filePath, xmlMode) {
  ...
  var query = "section[data-type], div[data-type], figure, table, *[id]";
...
Mapper.prototype.add = function(item) {
  ...
  if(id in this.map) {
    this.log("Mapper: duplicate ID", id);
    // console.log(item)
    // this.map[id] = item;
  } else {
    this.map[id] = item;
...
Mapper.prototype.find = function(element, filePath) {
  ...
  if(!element){
    this.log("Mapper: No Element Provided");
    return null;
  }
  ...
  if(id in this.map) {
    ...
    this.log("Mapper: Element not in Map:", id);
    return null;
  }
