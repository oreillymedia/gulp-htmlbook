...
function gulpIndexer(options) {
  ...
  var stream = through.obj(function(file, enc, done) {
    ...
    // Overide to replace *
    if(sort.children && sort.children.length){
      sort.children[0].title = formating.gentext["index symbols"];
    }
    
    indexFile.contents = new Buffer(JSON.stringify(sort.children, null, '  '));
    
    this.push(indexFile);

    if(settings.finished) {
      settings.finished(sort.children);
    }
